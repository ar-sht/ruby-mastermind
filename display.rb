...
module Display
  ...
  def display_header
    ...
    puts "The code maker will create a code in the form of four integers that looks like this:\n "
    ...
    puts "\u2191 This is the feedback the guesser receives"
    puts "#{RESULT_KEY[1]} means the number is in the correct position and #{RESULT_KEY[0]} means the number is in the code but in the wrong position."
    puts "\nIf you're playing on hard mode your feedback will just look like this:"
  ...
  def display_result(arr, hard = false)
    ...
    marked_hash = {
      'Number in correct position' => num_matches,
      'Number in wrong Position' => num_partials
    }
    if hard
      marked_hash.each { |key, val| puts "#{key}: #{val}" }
    else
